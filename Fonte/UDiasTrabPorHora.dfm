object x: Tx
  ...
  object HorasTrab: TIBQuery
    Database = DmFire.Banco
    Transaction = DmFire.ibtTransacao
    SQL.Strings = (
      'SELECT sum(horas_trab.qtdmin) AS QTDMIN'
      '  from horas_trab'
      
        '  inner join entrada_saida on entrada_saida.seqentsai = horas_tr' +
        'ab.seqentsai and'
      
        '                              entrada_saida.codvei = horas_trab.' +
        'codvei'
      '  where'
      '    entrada_saida.codmot = :CODMOT AND'
      '    horas_trab.data between :DATINI and :DATFIM')
    Left = 544
    Top = 176
    ParamData = <
      item
    ...
    object HorasTrabQTDMIN: TFloatField
      FieldName = 'QTDMIN'
      ProviderFlags = []
    end
  end
